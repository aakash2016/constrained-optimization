...
double del, r;//GLobal Var.
int M_, M, nc;//Specifies the total dimension we are working with.. //Global Var.
int restart;
...
double multi_f(const vector<double> &x){
	...
	//g[0]= (pow((x[0]-5),2) + pow((x[1]-5),2))/100 - 1.0;
	//g[1]= (pow((x[0]-5),2) + pow((x[1]-5),2))/82.81 - 1.0;//Normalizing/
	...
	*/	
	...
	// for feasible points no penalty.
	for(int i=0; i<nc; i++){	
		if(g[i] < 0.0){
			sum = sum + r*g[i]*g[i];
		}
...
int main(){
	...
	double r, c, track; //c = factor for updating r.
	int n, M_, nc, numseq; //numseq = number of sequences for penalty function method.
	...
	vector<double> sol(M);//0.836, 2.940
	vector<double> x_2(M);	
	...
	if(f.is_open()){
		for(int k = 1; k <= numseq; ++k){
			...
			cout << "initial function value is " << multi_f(x_ip) << endl;
			f << multi_f(x_ip) << endl;
			sol = DFP(x_ip);

			// TERMINATION CONDITION.		
			for(int j = 0; j < M; j++){x_2[j] = sol[j] - x_ip[j];}
			track = norm(x_2)/norm(x_ip);
			
			for(int i = 1; i <= M; ++i){cout << sol[i-1] << endl; x_ip[i-1] = sol[i-1];} //1st sequence.
			::r *= c;
			cout << endl;
			if(track <= 0.0001){break;}
		}
		f.close();
	}
	return 0;
